    ...
    private void onAnimar() {
        try {
            ...
            if (tablaPasos.getItems().isEmpty()) {
                ...
                List<FalsaPosicion.Step> pasos = new ArrayList<>();
                double raiz = modelo.resolver(funcion, x0, x1, tol, pasos);
                tablaPasos.setItems(FXCollections.observableArrayList(pasos));
            ...
            alert.setContentText("La animación para el método de Falsa Posición no está implementada aún.");
            alert.showAndWait();
