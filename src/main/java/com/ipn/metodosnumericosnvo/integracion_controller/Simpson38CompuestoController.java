...
import com.ipn.metodosnumericosnvo.metodos_integracion.Simpson38Compuesto;
...
public class Simpson38CompuestoController {

    @FXML private TextField txtTamano;
    @FXML private TextField txtH;
    @FXML private TextArea txtValores;
    @FXML private TableView<Simpson38Compuesto.Iteracion> tabla;
    @FXML private TableColumn<Simpson38Compuesto.Iteracion, Integer> colI;
    @FXML private TableColumn<Simpson38Compuesto.Iteracion, Double> colFx;
    ...
    private void onCalcular() {
        try {
            ...
            String[] lineas = txtValores.getText().split("\\s+|,");
            if (lineas.length != n) {
            ...
            for (int i = 0; i < n; i++) {
                f[i] = Double.parseDouble(lineas[i]);
            }

            List<Simpson38Compuesto.Iteracion> pasos = new ArrayList<>();
            double resultado = Simpson38Compuesto.resolver(f, h, pasos);

            ObservableList<Simpson38Compuesto.Iteracion> datos = FXCollections.observableArrayList(pasos);
